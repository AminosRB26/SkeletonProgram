    ...
    class Program
    {
        ...
        private static void GetItem(List<Item> items, string itemToGet, int currentLocation, ref bool stopGame)
        {
            ...
            if (canGet)
            {
                ...
                else if (subCommand == "win")
                {
                    ...
                    stopGame = true;
                    DisplayPlayerInstructions(stopGame);
                    return;
                }
        ...
        private static void PlayDiceGame(List<Character> characters, List<Item> Items, string otherCharacterName)
        {
            ...
            else
            {
                ...
                if (playerScore > otherCharacterScore || playerScore < otherCharacterScore)
                {
        ...
        private static void DisplayCommandList()
        {
            ...
            Console.WriteLine("");
            Console.WriteLine("attack, a command to attack a nearby character with a weapon");
            Console.WriteLine("get, a command to get an item nearby");
            Console.WriteLine("use, a command to use an iten in your inventory or nearby");
            Console.WriteLine("eat, a command to eat an edible item in your inventory or nearby");
            Console.WriteLine("drink, a command to drink from a drinkable item in your inventory or nearby");
            Console.WriteLine("go, a command to move/go in a certain direction which you must specify");
            Console.WriteLine("read, a command to read words on an item in your inventory or nearby");
            Console.WriteLine("examine, a command to examine an item in your inventory or nearby");
            Console.WriteLine("open, a command to open a door nearby");
            Console.WriteLine("close, a command to close an item nearby");
            Console.WriteLine("move, a command to move an item in your inventory or nearby to a specified location");
            Console.WriteLine("say, a command to say something");
            Console.WriteLine("playdice, a command to play a dice game with another character nearby");
            Console.WriteLine("instructions, a command to display the number of commands you have used(Doesn't count as another command)");
            Console.WriteLine("quit, a command to exit the game");
        }
            ...
            while (!stopGame)
            {
                ...
                switch (Command)
                {
                    ...
                    case "eat":
                        ...
                        break;
                    case "drink":
                        DrinkItem(items, instruction, characters[0].CurrentLocation);
                        AddPlayerInstruction();
                        break;
                    case "fill":
        ...
        private static void EatItem(List<Item> items, string itemToUse, int currentLocation)
        {
            int indexOfItem;
            indexOfItem = GetIndexOfItem(itemToUse, -1, items);
            ...
            if (indexOfItem == -1)
            {
                Console.WriteLine("You can't find " + itemToUse + " to eat");
            }
            else if (items[indexOfItem].Location == Inventory && items[indexOfItem].Status.Contains("edible"))
            {
                Console.WriteLine("You have eaten " + items[indexOfItem].Name);
                items.RemoveAt(indexOfItem);
            }
            else
            {
                Console.WriteLine("There is no " + items[indexOfItem].Name + " to eat");
            }
        }

        private static void DrinkItem(List<Item> items, string itemToDrink, int currentLocation)
        {
            
            int indexOfItem;
            indexOfItem = GetIndexOfItem(itemToDrink, -1, items);

            if (indexOfItem == -1)
            {
                Console.WriteLine("You can't find " + itemToDrink + " to drink from");
            }
            else if(items[indexOfItem].Location == Inventory && items[indexOfItem].Status.Contains("drinkable"))
            {
                Console.WriteLine("You have drank from the " + itemToDrink);
                ChangeStatusOfItem(items, indexOfItem, "nondrinkable");
            }
            else
            {
                Console.WriteLine("You cannot drink from the " + itemToDrink);
            }
        }
        ...
        private static void AddPlayerInstruction()
        {
            Instructions++;
        }
